...
class Build:
    ...
    WARNING: Can deadlock if:
    - Dependency tree depth × max deps per node > max_workers
    - Any node has > max_workers dependencies
    Increase max_workers if you encounter deadlocks.
    """
    ...
    def _find_rule_fn(self, target: str) -> RuleFn | None:
        ...
        for target in targets:
            ...
            with self._lock:
                ...
                if target in self._done:
                    ...
                    future = self._executor.submit(self._build_target, target)
    ...
    def _build_target(self, target: str) -> None:
        ...
        try:
            rule_fn = self._find_rule_fn(target)
            if rule_fn is None:
    ...
    def run(self, target: str) -> None:
        ...
        with ThreadPoolExecutor(
            max_workers=self._max_workers, thread_name_prefix="Builder"
        ) as executor:
            self._executor = executor
            try:
                self.need(target)
            except KeyboardInterrupt:
                logger.info("Build interrupted, shutting down...")
