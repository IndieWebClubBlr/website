...
class FeedEntry:
    ...
    def __init__(
        self, title: str, link: str, published: datetime, feed_title: str, tags: [str]
    ):
        self.title = title
        self.link = link
...
def parse_feed(feed_title: str, feed_content: str) -> List[FeedEntry]:
    ...
    try:
        ...
        for entry in parsed_feed.entries[
            ...
            tags = [
                tag.get("label") or tag.get("term") for tag in getattr(entry, "tags", [])
            ]
...
<body>
    <header>
        <h1>{{title}}</h1>
        <ul>
            <li class="gen-time"><time datetime="{{generated_date}}">{{generated_date}}</time></li>
            <li class="entry-count">{{total_entries}}</li>
            <li class="feed-count">{{total_feeds}}</li>
        </ul>
    </header>
    ...
    <main>
      <section id="recent-posts">
        ...
        <article class="entry">
            ...
            <ul>
                <li class="feed-title">Blog: {{feed_title}}</li>
                <li class="pub-time">Published: <time datetime="{{published_machine}}">{{published_human}}</time></li>
                <li class="tags">Tags: <ul>{{#tags}}<li>{{.}}</li>{{/tags}}</ul></li>
            </ul>
        </article>
        {{/entries}}
      </section>
    ...
    <footer>
        <ul>
            <li>Generated by OPML Feed Aggregator</li>
        </ul>
    </footer>
</body>
</html>"""
